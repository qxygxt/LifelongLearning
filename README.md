...
## 1.1 形式化定义
终身机器学习(LML)是一个连续不断的学习过程，在这个过程中，学习者已经完成了N个学习任务，T1，T2，..TN。当面对(N+1)的任务TN+1和它的数据DN+1时，学习者可以利用其知识库(知识库)(内存)中的先验知识来帮助学习TN+1。知识库存储并维护在过去学习N个任务时所学习和积累的知识。在学习了TN+1之后，知识库更新了从TN+1中学习的(中间和最后的)结果。

陈等人[6]的这一定义表明，LML的关键特征是 1)持续学习，2) 知识积累在知识库(知识库)中，3)利用知识库中的知识来帮助未来的学习。
这些特点使它有别于其他相关的学习任务，如transfer learning 和multi-task learning。
## 1.2 概念辨析
...
## 1.4 如何评价LML模型的好坏（3个指标）
1. 准确率：也就是学习完最后一个任务task T之后，模型在task 1到task T-1上的平均正确率。

2. 记忆能力：这个指标衡量了模型学习完之后一个任务task T后，在之前task上的表现。计算公式为正确率之差，此指标一般为负数，因为模型会忘记之前的task。

3. 迁移能力：此指标衡量了模型在学习完一个任务后，在未学习的task上的表现，一般和随机初始化的模型进行对比。
